...
### Added
- PSM Party Mode full workflow (3 steps: agent-loading → discussion → graceful-exit)
- PSM Party Mode SKILL.md entry point
- `_memory/` sidecars: `sre-sidecar/` + `tech-writer-sidecar/` (BMAD compliance)
- `psm/knowledge-base/` → `psm/references/` aligned with canonical folder names
- Party mode registered in `workflow-manifest.csv` and `module-help.csv`
- BMAD compliance: 98% → ~95% (accuracy — discovered actual gaps after deep comparison)

### Changed
- `_bmad/psm/` vs `_production-system/psm/` identified as two different modules
...
### Added
- 5 output templates (review-architecture, setup-new-service, mlops-deployment, quick-diagnose, quick-review)
- 8 `step-01b-continue.md` session continuation files (BMAD v6.2 compliance)
- BMAD v6.2 state tracking: `stepsCompleted`, `lastStep`, `lastSaved` frontmatter in all workflow outputs
- Checkpoint protocol: progressive save + menu after each step in all workflows
- Entry point continuation detection in all 8 workflow.md files
- `_production-system-output/` populated with sample outputs and quick-start guide

### Fixed
- Skill path mismatch: aligned `skill_loads` references with skill-manifest.csv conventions
- BMAD compliance: 92% → 98%
